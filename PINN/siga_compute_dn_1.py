...
num_profiles = 10			# Numero di profili da generare
...
with open(dataset_file, 'w') as output_file:
	for _ in range(num_profiles):
		...
		shutil.copy('/home/squillace/DEEPONET/HOMEMADE/resources/NACA0012_39.dat', './')	# Copiare la geometria di base voluta

		fileinput = 'eval_obj.in'
		lines = []
		lines.append('BASELINE_AIRFOIL=NACA0012_39.dat\n')	# Sostituire con il nome del file dati corretto
		param = []

		for i in range(20):			
		    value = random.uniform(0, 0)	# Se si vuole mantenere la geometria costante per tutto il dataset
		    #value = random.uniform(-0.5, 0.5) 	# Per generare geometrie modificate sfruttando WG2AER
		    lines.append(f'X({i}) = {value:.6f}\n')
		    param.append(f'{value:.6f}')		
		...
		for i in range(1, (n//2)+1):
			if yo[i - 1] > yo[n - i]:
				...
				output_file.write(' '.join('1' for _ in range(20)) + ' 1\n')
...
#		output_file.write(data_x + '   ' + data_y + f' {alpha:.6f} ' + siga + f' {cl:.6f}\n')
		...
		print('######## Profili completati:', (_+1), '/', num_profiles)
...
with open(dataset_file, "r") as input_file, open(filtered_file, "w") as output_file:
